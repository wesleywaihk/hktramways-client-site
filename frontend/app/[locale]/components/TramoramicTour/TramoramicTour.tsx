...
export default function TramoramicTour({
  ...
  const imgClassName =
    "absolute right-0 top-1/2 w-[71vmin] md:w-[85%] pt-[5.7vmin] px-[5.7vmin] pb-[14.6vmin] md:pt-[32px] md:px-[32px] md:pb-[94px] lg:pt-[40px] lg:px-[40px] lg:pb-[100px] bg-white [container-type:inline-size]";
    ...
    <section className="borderless relative bg-red-dark py-0 py-20 lg:py-24 h-auto md:h-[calc(100dvh-52px)] lg:h-[calc(100dvh-100px)] flex">
      <div className="flex flex-col md:flex-row items-center gap-12 md:gap-8 lg:gap-16 px-5 lg:px-6 max-w-[1200px] mx-auto">
        <div className="relative w-[87vmin] md:w-full min-h-[84.5vmin] md:min-h-0 md:flex-1 mt-[12vmin] mb-[6vmin] md:my-0">
          {supportImage2 && (
            <div
              className={`${imgClassName} z-0 -translate-x-[11%] -translate-y-[60%] shadow-lg`}
            >
              <Img
                src={supportImage2.url}
                alt=""
          ...
          {supportImage1 && (
            <div
              className={`${imgClassName} z-10 -translate-x-[18%] -translate-y-[42%] shadow-lg`}
            >
              <Img
                src={supportImage1.url}
                alt=""
                className="w-full aspect-square object-cover"
              />
            </div>
          ...
          <div
            className={`${imgClassName} z-20 -translate-y-1/2 -translate-y-1/2 shadow-xl`}
          >
            <ResponsiveImg
              bannerImage={{
                id: mainImage?.id ?? 0,
                altText: mainImage?.alternativeText ?? null,
                bannerD: mainImage ?? null,
                bannerM: null,
              }}
