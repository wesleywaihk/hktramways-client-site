...
export function useFloatingCircle(containerRef: RefObject<HTMLElement | null>) {
   ...
   * settles `settleMs` later, re-derive hover state from the cursor's last
   * known position: find whatever element is now under it and replay a
   * mousemove there, so a cursor that never moved still picks the circle
   * back up over whatever landed underneath it.
   */
  const hideForTransition = useCallback((settleMs: number) => {
    isHoveringRef.current = false;
    setVisible(false);
    if (recheckTimer.current != null) clearTimeout(recheckTimer.current);
